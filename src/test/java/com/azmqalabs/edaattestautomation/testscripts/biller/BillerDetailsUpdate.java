...
public class BillerDetailsUpdate {
	
	// DECLARATION SECTION
	  static String TestDataTab="BillerDetailsUpdate";  
	  ...
	  DBUtil DBUtil;
	    ...
	    public BillerDetailsUpdate(Map<Object, Object> testdatamap){
	    	this.testdatamap=testdatamap;
	        this.TestScriptID=testdatamap.get("TestScriptID").toString();
	        this.sBrowserTestData=testdatamap.get("Browser").toString();
	        this.UserName=testdatamap.get("UserName").toString();
	  	    this.Password=testdatamap.get("Password").toString();
	  	    this.Billername=testdatamap.get("Billername").toString();
	  	    this.MonthlyLimits=testdatamap.get("MonthlyLimits").toString();
	  	    this.Transfers=testdatamap.get("Transfers").toString();
	  	    this.AZMFees=testdatamap.get("AZMFees").toString();
	  	    this.SadadFees=testdatamap.get("SadadFees").toString();
	  	    this.CorporateName=testdatamap.get("CorporateName").toString();
	  	    this.MaximumAmt=testdatamap.get("MaximumAmt").toString();
	    }
	    @Override
...
//	    @Parameters({ "username", "password" })
	    ...
	    public void myTest() throws InterruptedException, Exception{
	        try{
	        	...
	        	Log.ReportEvent("INFO", "Step3 : Verify Biller details Updated Successfully");
	        	Pages.BillerSystemMangement.UpdateBillerdetails(Billername, MonthlyLimits, Transfers, AZMFees, SadadFees, CorporateName, MaximumAmt);
