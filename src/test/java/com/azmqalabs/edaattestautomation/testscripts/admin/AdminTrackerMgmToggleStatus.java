...
public class AdminTrackerMgmToggleStatus {
	
	// DECLARATION SECTION
	  static String TestDataTab="AdmTrackerBillerMgmToggle";  
	  ...
	  DBUtil DBUtil;
	    ...
	    public AdminTrackerMgmToggleStatus(Map<Object, Object> testdatamap){
	    ...
	    public void myTest() throws InterruptedException, Exception{
	        try{
				...
				Log.ReportEvent("INFO", "Step4 :  Verify the status toggel  & Verify Activate Biller / Corporate button ");
				Pages.AdminTrackerBillerManagementPage.verifyTrackerBillerMgmToggle(testdatamap);

				
				if(record==false) {
					Log.ReportEvent("INFO", "Step5 : Verify User Able to Register or Sign Up Successfully");
					...
					Log.ReportEvent("INFO", "Step6 : Verify Admin Login is Suceessfull");
					Pages.AdminLoginPage.clickOnBillerLogin();
					Pages.AdminLoginPage.LoginToApplication(testdatamap);
					
					Log.ReportEvent("INFO", "Step7 : Naviagate to Not approved Biller Management ");
					Pages.AdminSystemMangement.navigateNotApprovedBillerManagement();
					
					Log.ReportEvent("INFO", "Step8 : Save Biller Details as Tracker");
		        	Pages.AdminNotApprovedBillerManagement.CreateTrackerBill(testdatamap);
		        	
		        	Log.ReportEvent("INFO", "Step9 : Navigate to tracker management");
		        	Pages.AdminSystemMangement.navigateTrackerBillerManagement();

					Log.ReportEvent("INFO", "Step10 : Verify the status toggel  & Verify Activate Biller / Corporate button ");
					Pages.AdminTrackerBillerManagementPage.verifyTrackerBillerMgmToggle(testdatamap);
				}
