...
public class AdminTrackerMgmGridView {
	
	// DECLARATION SECTION
	  static String TestDataTab="AdmTrackerBillerMgmGridView";  
	  ...
	  DBUtil DBUtil;
	    ...
	    public AdminTrackerMgmGridView(Map<Object, Object> testdatamap){
	    ...
	    public void myTest() throws InterruptedException, Exception{
	        try{
				...
				Log.ReportEvent("INFO", "Step2 : Verify Admin Login is Suceessfull");
				Pages.AdminLoginPage.LoginToApplication(testdatamap);

				Log.ReportEvent("INFO", "Step3 : Naviagate to Not approved Biller Management ");
				Pages.AdminSystemMangement.navigateTrackerBillerManagement();

				Log.ReportEvent("INFO", "Step4 : Verify Grid view details ");
				Pages.AdminTrackerBillerManagementPage.VerifyGridViewTrackerManagement(testdatamap);
				
				Log.ReportEvent("INFO", "Step5 : Verify that Total Corporate Clients is clickable ");
	        	Pages.AdminTrackerBillerManagementPage.verifyCorporateClientIDClickable();

				
				if(record==false) {
					Log.ReportEvent("INFO", "Step6 : Verify User Able to Register or Sign Up Successfully");
					Pages.AdminLoginPage.Logout(EdaatOR.Logout);
					Pages.AdminLoginPage.clickOnBillerLogin();
					Pages.AdminSignUpPage.AdminRegisterWithNewUserID(testdatamap);

					Log.ReportEvent("INFO", "Step7 : Verify Admin Login is Suceessfull");
					Pages.AdminLoginPage.clickOnBillerLogin();
					Pages.AdminLoginPage.LoginToApplication(testdatamap);
					
					Log.ReportEvent("INFO", "Step8 : Naviagate to Not approved Biller Management ");
					Pages.AdminSystemMangement.navigateNotApprovedBillerManagement();
					
					Log.ReportEvent("INFO", "Step9 : Save Biller Details as Tracker");
		        	Pages.AdminNotApprovedBillerManagement.CreateTrackerBill(testdatamap);
		        	
		        	Log.ReportEvent("INFO", "Step10 : Navigate to tracker management");
		        	Pages.AdminSystemMangement.navigateTrackerBillerManagement();

					Log.ReportEvent("INFO", "Step11 : Verify Grid view details");
					Pages.AdminTrackerBillerManagementPage.VerifyGridViewTrackerManagement(testdatamap);
					
					Log.ReportEvent("INFO", "Step12 : Verify that Total Corporate Clients is clickable ");
		        	Pages.AdminTrackerBillerManagementPage.verifyCorporateClientIDClickable();
				}

				Log.ReportEvent("INFO", "Step: Logout  Suceessful");
				Pages.AdminLoginPage.Logout(EdaatOR.Logout);
